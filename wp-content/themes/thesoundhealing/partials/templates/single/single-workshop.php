...
$ws_guests     = get_field('ws_guests',     $post_id);
...
$gallery_subs = array_values(array_filter(
    [$ws_gal_2, $ws_gal_3, $ws_gal_4, $ws_gal_5, $ws_gal_6, $ws_gal_7, $ws_gal_8, $ws_gal_9],
...
$ws_ins_youtube   = get_field('ws_instructor_youtube',   $post_id);
$ws_ins_tiktok    = get_field('ws_instructor_tiktok',    $post_id);
...
// ── Status badge ──
$status_map = [
    'open'     => ['label' => __('Còn chỗ', 'monamedia'),       'color' => '#2e7d32', 'bg' => '#e8f5e9'],
    'limited'  => ['label' => __('Sắp hết chỗ', 'monamedia'),   'color' => '#e65100', 'bg' => '#fff3e0'],
    'closed'   => ['label' => __('Hết chỗ', 'monamedia'),        'color' => '#c62828', 'bg' => '#ffebee'],
    'upcoming' => ['label' => __('Sắp diễn ra', 'monamedia'),    'color' => '#1565c0', 'bg' => '#e3f2fd'],
];
                ...
                <div class="row">
                                            ...
                                            ?>
                                                <?php if ($show_all) : ?>
                                                    <div class="relative flex-1 overflow-hidden">
                                                        ...
                                                        <a href="<?php echo esc_url($t['url']); ?>" data-fancybox="gallery-ws" class="hidden" aria-hidden="true"></a>
                                                        <?php foreach ($extra as $ex) : ?>
                                                            <a href="<?php echo esc_url($ex['url']); ?>" data-fancybox="gallery-ws" class="hidden" aria-hidden="true"></a>
                                                        <?php endforeach; ?>
                                                        <button data-fancybox-trigger="gallery-ws"
                                    ...
                                    <div class="flex gap-5 items-start">
                                        <?php if (!empty($ws_ins_image['url'])) : ?>
                                            <div class="size-16 rounded-full overflow-hidden shrink-0">
                                                <img src="<?php echo esc_url($ws_ins_image['url']); ?>"
                                                    class="block w-full h-full object-cover"
                                        ...
                                        <div>
                                            ...
                                            <?php endif; ?>
                                            <?php if ($ws_ins_instagram || $ws_ins_facebook || $ws_ins_youtube || $ws_ins_tiktok) : ?>
                                                <div class="flex gap-2 mt-4">
                                                    ...
                                                    <?php endif; ?>
                                                    <?php if ($ws_ins_youtube) : ?>
                                                        <a href="<?php echo esc_url($ws_ins_youtube); ?>" target="_blank" rel="noopener noreferrer"
                                                            class="size-8 flex items-center justify-center rounded-full border border-[#e4e2dd] text-[#414847] hover:border-[#4e635a] hover:text-[#4e635a] transition-colors">
                                                            <svg class="size-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    ...
                                                    <?php endif; ?>
                                                    <?php if ($ws_ins_tiktok) : ?>
                                                        <a href="<?php echo esc_url($ws_ins_tiktok); ?>" target="_blank" rel="noopener noreferrer"
                                                            class="size-8 flex items-center justify-center rounded-full border border-[#e4e2dd] text-[#414847] hover:border-[#4e635a] hover:text-[#4e635a] transition-colors">
                                                            <svg class="size-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="col col-5 max-md:hidden">
                        <div id="form-dang-ky"
                            ...
                            <?php
                            $meta_rows = [];
                            if ($ws_date)     $meta_rows[] = ['label' => __('NGÀY', 'monamedia'),            'value' => $ws_date,     'type' => 'text',     'icon' => 'calendar'];
                            if ($ws_time)     $meta_rows[] = ['label' => __('THỜI GIAN', 'monamedia'),       'value' => $ws_time,     'type' => 'text',     'icon' => 'clock'];
                            if ($ws_duration) $meta_rows[] = ['label' => __('THỜI LƯỢNG', 'monamedia'),      'value' => $ws_duration, 'type' => 'text',     'icon' => 'clock'];
                            if ($ws_guests)   $meta_rows[] = ['label' => __('SỐ LƯỢNG KHÁCH', 'monamedia'),  'value' => $ws_guests,   'type' => 'text',     'icon' => 'users'];
                            if ($ws_location) $meta_rows[] = ['label' => __('ĐỊA ĐIỂM', 'monamedia'),        'value' => $ws_location, 'type' => 'location', 'icon' => 'location'];
                                ...
                                <div class="p-6 max-md:p-4 border-b border-[#e4e2dd]">
                                    <?php if ($ws_price) : ?>
                                        <div class="flex items-baseline gap-1 mb-4">
                                            <span class="font-title text-pri text-[28px] max-md:text-[20px] font-bold"><?php echo esc_html($ws_price); ?></span>
                                            <?php if (strtolower(trim($ws_price)) !== 'liên hệ') : ?><span class="text-[#717171] text-[14px]"><?php esc_html_e('/ người', 'monamedia'); ?></span><?php endif; ?>
                                        </div>
                                    <?php endif; ?>
                                                ...
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <div class="flex flex-col justify-center min-h-10">
                                                        ...
                                                        <?php if ($ws_spots !== null) :
                                                            ...
                                                            <?php else : ?>
                                                                <span class="inline-flex items-center gap-1.5 bg-[#fef9c3] text-[#854d0e] text-[12px] font-semibold px-2 py-1 rounded-[4px]"><?php echo sprintf(__('Còn %d suất', 'monamedia'), $ws_spots); ?></span>
                                                            <?php endif;
                                                        elseif ($ws_capacity) : ?>
                            ...
                            <!-- CF7 Form / Buy Now -->
                            <div id="ws-form-inner" class="flex p-6 max-md:p-4 flex-col gap-3">
                                ...
                                if ($ws_cf7_id) : ?>
                                    <div class="cf7-workshop" <?php if ($_ws_has_wc) : ?> data-buy-url="<?php echo esc_url($_ws_buy_url); ?>" <?php endif; ?>>
