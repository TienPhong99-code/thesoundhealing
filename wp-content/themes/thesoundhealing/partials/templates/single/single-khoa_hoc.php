...
$kh_st_about      = get_field('kh_sectitle_about',      $post_id) ?: __('Về khóa học', 'monamedia');
$kh_st_benefits   = get_field('kh_sectitle_benefits',   $post_id) ?: __('Mục tiêu & Lợi ích', 'monamedia');
$kh_st_roadmap    = get_field('kh_sectitle_roadmap',    $post_id) ?: __('Lộ trình học', 'monamedia');
$kh_st_receive    = get_field('kh_sectitle_receive',    $post_id) ?: __('Lợi ích khóa học', 'monamedia');
$kh_st_instructor = get_field('kh_sectitle_instructor', $post_id) ?: __('Người hướng dẫn', 'monamedia');
...
$ins_youtube   = get_field('instructor_youtube',   $post_id);
$ins_tiktok    = get_field('instructor_tiktok',    $post_id);
...
<?php get_template_part('partials/components/breadcrumb', null, [
    'links' => [
        ...
        ['title' => __('Khóa học', 'monamedia'),  'url' => home_url('/khoa-hoc'), 'is-active' => false],
        ['title' => get_the_title(), 'url' => '',                'is-active' => true],
    ],
]); ?>

<div class="page-khoa-hoc section-pd-t max-md:!pt-4">

    <div class="container">

        <!-- ── CONTENT + ASIDE ──────────────────────────────────────────── -->
        <section class="sec-kh-content">
            <div class="relative">
                <div class="row">
                    ...
                    <div class="col col-7 max-md:!w-full">
                        ...
                        <div class="flex flex-col divide-y divide-[#e4e2dd]">
                            ...
                            <?php if (!empty($bn_items)) : ?>
                                <div class="py-10">
                                    <h2 class="font-title text-pri text-[24px] leading-[32px] font-bold mb-6">
                                        <?php echo esc_html($kh_st_benefits); ?>
                                    </h2>
                            ...
                            <?php if (!empty($rm_items)) : ?>
                                <div class="py-10">
                                    <h2 class="font-title text-pri text-[24px] leading-[32px] font-bold mb-8">
                                        <?php echo esc_html($kh_st_roadmap); ?>
                                    </h2>
                                    <div class="flex flex-col divide-y divide-[#e4e2dd]">
                            ...
                            <?php if (!empty($receive_items)) : ?>
                                <div class="py-10">
                                    <h2 class="font-title text-pri text-[24px] leading-[32px] font-bold mb-6">
                                        <?php echo esc_html($kh_st_receive); ?>
                                    </h2>
                                    <div class="grid grid-cols-2 max-md:grid-cols-1 gap-[1px] bg-[#e4e2dd] border border-[#e4e2dd] rounded-[8px] overflow-hidden">
                            ...
                            <?php if ($ins_name) : ?>
                                <div class="py-10">
                                    <h2 class="font-title text-pri text-[24px] leading-[32px] font-bold mb-6">
                                        <?php echo esc_html($kh_st_instructor); ?>
                                    </h2>
                                    <div class="flex gap-5 items-start">
                                        ...
                                        <div>
                                            ...
                                            <?php endif; ?>
                                            <?php if ($ins_instagram || $ins_facebook || $ins_youtube || $ins_tiktok) : ?>
                                                <div class="flex gap-2 mt-4">
                                                    ...
                                                    <?php endif; ?>
                                                    <?php if ($ins_youtube) : ?>
                                                        <a href="<?php echo esc_url($ins_youtube); ?>" target="_blank" rel="noopener noreferrer"
                                                            class="size-8 flex items-center justify-center rounded-full border border-[#e4e2dd] text-[#414847] hover:border-[#4e635a] hover:text-[#4e635a] transition-colors">
                                                            <svg class="size-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    ...
                                                    <?php endif; ?>
                                                    <?php if ($ins_tiktok) : ?>
                                                        <a href="<?php echo esc_url($ins_tiktok); ?>" target="_blank" rel="noopener noreferrer"
                                                            class="size-8 flex items-center justify-center rounded-full border border-[#e4e2dd] text-[#414847] hover:border-[#4e635a] hover:text-[#4e635a] transition-colors">
                                                            <svg class="size-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="col col-5 max-md:hidden">
                        <div id="form-dang-ky"
                            ...
                            <?php
                            $meta_rows = [];
                            if ($start_date)   $meta_rows[] = ['label' => __('KHAI GIẢNG', 'monamedia'),  'value' => $start_date,   'type' => 'text',     'icon' => 'calendar'];
                            if ($kh_time)      $meta_rows[] = ['label' => __('THỜI GIAN', 'monamedia'),   'value' => $kh_time,      'type' => 'text',     'icon' => 'clock'];
                            if ($duration)     $meta_rows[] = ['label' => __('THỜI LƯỢNG', 'monamedia'),  'value' => $duration,     'type' => 'text',     'icon' => 'clock'];
                            if ($kh_capacity)  $meta_rows[] = ['label' => __('SỐ HỌC VIÊN', 'monamedia'), 'value' => $kh_capacity,  'type' => 'text',     'icon' => 'users'];
                            if ($kh_location)  $meta_rows[] = ['label' => __('ĐỊA ĐIỂM', 'monamedia'),   'value' => $kh_location,  'type' => 'location', 'icon' => 'location'];
                            $has_spots = $kh_spots !== null;
                            if ($price || !empty($meta_rows) || $has_spots) : ?>
                                <div class="p-6 max-md:p-4 border-b border-[#e4e2dd]">
                                    <?php if ($price) : ?>
                                        <div class="flex items-baseline gap-1 mb-4">
                                            <span class="font-title text-pri text-[28px] max-md:text-[20px] font-bold"><?php echo esc_html($price); ?></span>
                                            <?php if (strtolower(trim($price)) !== 'liên hệ') : ?><span class="text-[#717171] text-[14px]"><?php esc_html_e('/ khóa học', 'monamedia'); ?></span><?php endif; ?>
                                        </div>
                                    <?php endif; ?>
                                            ...
                                            <?php if ($has_spots) : ?>
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <div class="flex flex-col justify-center min-h-10">
                                                        ...
                                                        <?php else : ?>
                                                            <span class="inline-flex items-center gap-1.5 bg-[#fef9c3] text-[#854d0e] text-[12px] font-semibold px-2 py-1 rounded-[4px]"><?php echo esc_html(sprintf(__('Còn %d suất', 'monamedia'), $kh_spots)); ?></span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
